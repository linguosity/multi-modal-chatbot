            ...
            {group.items?.map((item) => (
              <SidebarMenuItem key={item.url || item.title}>
                <SidebarMenuButton /* ...props... */ >
                  ...
                  {isCollapsed
                    ? <item.collapsedIcon className="w-5 h-5" />
...
export default function DashboardLayout({
  ...
  useEffect(() => {
    console.log(`Dashboard Layout: useEffect - isLoading: ${isLoading}, user: ${!!user}`)
    if (!isLoading && !user) {
      console.log('Dashboard Layout: No user found, redirecting to /auth')
  ...
  console.log(`Dashboard Layout: Render check - isLoading: ${isLoading}, user: ${!!user}`)
  if (isLoading || !user) {
