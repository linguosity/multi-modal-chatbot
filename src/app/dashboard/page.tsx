...
export default async function DashboardPage() {
  ...
  // Use the centralized client wrapper
  const supabase = await createClient()  
  const { data: { user }, error } = await supabase.auth.getUser()  
  
  console.log(`Dashboard Page: User authenticated: ${!!user}, Error: ${error?.message || 'none'}`)
  
  // Redirect if not authenticated
  if (!user) {
    console.log('Dashboard Page: No user found, redirecting to /auth')
    redirect('/auth');
  }
