...
export const useBatchReportUpdater = (initialReport: SpeechLanguageReport) => {
   ...
   */
  const processText = useCallback(async (input: string, section?: string) => {
    ...
    try {
      ...
      const { status, requestId, report: updatedReport, message } = await response.json();
      
      console.log(`Report processing ${status}: Request ID: ${requestId}`);
      console.log(`Message: ${message}`);
      
      if (status === 'completed' && updatedReport) {
        // Update completed immediately with text editor tool
        ...
        setProgress(100);
        console.log('Report state updated with new data');
        return requestId;
      } else {
  ...
  const processPdf = useCallback(async (pdfData: string) => {
    ...
    try {
      ...
      console.log(`PDF processing ${status}: Request ID: ${requestId}`);
      console.log(`Message: ${message}`);
      ...
      if (status === 'completed' && updatedReport) {
        ...
        setBatchStatus('completed');
        setProgress(100);
        console.log('Report updated successfully with PDF processing via text editor tool');
        return requestId;
      } else {
        throw new Error(message || 'Unexpected response from server');
      }
