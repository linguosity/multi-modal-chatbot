...
interface EditorPanelProps {
  ...
  error: string | null;
  success: string | null;
  // Ensure handleSubmit passed from parent handles API logic
  handleSubmit: (e: React.FormEvent) => Promise<void>;
  onExportHtml: () => void;
  onClearReport: () => void;
  onViewJson: () => void;
  report: SpeechLanguageReport; // Receives current report state
  // Ensure onPdfUpload handles API call and potentially returns batch info
  onPdfUpload: (pdfData: string) => Promise<any>; // Return type might need adjustment
...
export const EditorPanel: React.FC<EditorPanelProps> = ({
  ...
  error,
  success,
  handleSubmit, // This prop should trigger the API call in the parent (ReportEditor)
  onExportHtml,
  onClearReport,
  onViewJson,
  report,
  ...
  const handlePdfSubmit = async (pdfData: string) => {
    ...
    try {
      ...
      setBatchStatus({ active: false, batchId: '', sections: [] }); // Reset batch status on error
    }
  };

  // Handle standard text submit - trigger processing via parent prop
  const handleSubmitRequest = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!inputText.trim() || isUpdating) return;

    // Call the handleSubmit prop passed from ReportEditor
    // ReportEditor's handleSubmit should handle the API call and subsequent state updates (including batch status if applicable)
    await handleSubmit(e);

    // Optionally close editor after standard submission if desired (parent might handle success state)
    // if (!shouldUseBatchMode && success) { // Check success state from props
    //   setEditorOpen(false);
    // }
    // We don't set batchStatus here; ReportEditor's handleSubmit should manage that based on API response
     // Clear input text ONLY if submission was likely successful (or let parent clear it via state update)
     setInputText(''); // Clear text after submission attempt
     // Closing the dialog might be better handled based on success/error state change from parent
     // setEditorOpen(false);
  };
