...
export default function ReportEditor({ reportId }: ReportEditorProps) {
  ...
  // Update batch hook when initial report changes
  useEffect(() => {
    if (state.initialReport) {
      console.log("initialReport loaded, updating hook state", state.initialReport);
  ...
  }, [state.initialReport, batchUpdaterHook.setReport]);
